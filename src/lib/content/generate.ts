...
You are given: the verified product analysis, the brand profile (voice AND visual identity), the strategy, the personas, and a list of briefs. Write one post per brief.
...
- Match the product's look-and-feel in copy: honour visual_style, imagery_style, visual_dos and visual_donts. Describe the real UI/screens when demoing — never invent a generic stock aesthetic.
- When brand.visual_identity.colors or typography are present, treat them as constraints (reference the product's own colours and type vibe; do not invent a different palette).
- When brand.visual_identity is thin or missing, stay plain and product-specific — do not invent a random aesthetic.
...
export async function generateContent(scope: TenantScope, input: GenerateContentInput): Promise<GenerateContentResult> {
  ...
  const { data, costUsd: batchCost, degraded } = await generateObject({
    ...
    context: { project_name: project.name, analysis: { one_liner: analysis.one_liner, what_it_does: analysis.what_it_does, category: analysis.category, features: analysis.features, not_capabilities: analysis.not_capabilities, differentiators: analysis.differentiators, problem_solved: analysis.problem_solved, tech_stack: analysis.tech_stack, screens: analysis.screens }, brand: { voice: brand.voice, tone_attributes: brand.tone_attributes, messaging_pillars: brand.messaging_pillars, words_to_use: brand.words_to_use, words_to_avoid: brand.words_to_avoid, ctas: brand.ctas, emoji_policy: brand.emoji_policy, terminology: brand.terminology, visual_style: brand.visual_style, design_language: brand.design_language, imagery_style: brand.imagery_style, graphic_style: brand.graphic_style, brand_personality: brand.brand_personality, visual_dos: brand.visual_dos, visual_donts: brand.visual_donts, content_dos: brand.content_dos, content_donts: brand.content_donts, visual_identity: { brand_name: brand.brand_name || null, colors: { primary: brand.primary_color || null, secondary: brand.secondary_color || null, accent: brand.accent_color || null, background: brand.background_color || null, text: brand.text_color || null }, typography: { heading: brand.heading_font || null, body: brand.body_font || null }, logo_url: brand.logo_url || null, thin: !(brand.primary_color || brand.logo_url || brand.heading_font || brand.body_font) } }, strategy: { positioning: strategy.positioning, value_proposition: strategy.value_proposition, cta_strategy: strategy.cta_strategy }, personas: personas.map((p) => ({ name: p.name, role: p.role, pain_points: p.pain_points, objections: p.objections, tone_preference: p.tone_preference })), existing_hooks: existing.slice(-40).map((c) => c.hook), briefs },
