...
export async function upsertUser(scope: TenantScope, user: Omit<User, 'created_at'> & { created_at?: string }): Promise<User> {
  const existing = await db().get(scope, 'users', user.id);
  if (existing) return db().update(scope, 'users', user.id, { email: user.email, name: user.name, avatar_url: user.avatar_url, is_admin: user.is_admin });
...
/**
 * Idempotent enqueue with an optional logical key. A logical key is important
 * for durable chains that legitimately contain multiple jobs of the same type
 * (for example content batches 0, 3, 6). The key prevents duplicate requests
 * for the same batch without treating the currently-running batch as a reason
 * to suppress the next batch.
 */
export async function enqueueOnce(
  scope: TenantScope,
  type: JobType,
  payload: Record<string, unknown>,
  opts: { projectId?: Uuid | null; runAfter?: string; maxAttempts?: number; dedupeKey?: string } = {},
  ...
  try {
    const open = await db().find(scope, 'jobs', {
      ...
      orderBy: 'created_at', direction: 'asc', limit: 50,
    });
    const live = open.find((j) => {
      if (isStalled({ status: j.status, lockedAt: j.locked_at, updatedAt: j.updated_at })) return false;
      if (!opts.dedupeKey) return true;
      return String(j.payload.idempotencyKey ?? '') === opts.dedupeKey;
    });
...
export async function recordError(scope: TenantScope, input: { projectId?: Uuid | null; runId?: Uuid | null; scope: string; message: string; remedy?: string | null; fatal?: boolean }): Promise<AutomationError> {
  return db().insert(scope, 'automation_errors', { id: newId(), project_id: input.projectId ?? null, automation_run_id: input.runId ?? null, scope: input.scope, message: input.message, remedy: input.remedy ?? null, fatal: input.fatal ?? false, resolved: false, created_at: nowIso() });
}
