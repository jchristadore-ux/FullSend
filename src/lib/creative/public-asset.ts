...
/**
 * Materialises locally-rendered SVG into a real PNG in Supabase Storage.
 * Instagram cannot fetch a browser blob, server-local path, or inline SVG
 * reference, so publishing is allowed to use only a durable public URL.
 */
export async function ensurePublicCreative(asset: CreativeAsset): Promise<CreativeAsset> {
  ...
  if (!env.supabase.url || !env.supabase.serviceRoleKey) {
    throw new FullSendError('storage_not_configured', 'Public creative storage is not configured', {
      retryable: false,
      remedy: 'Configure NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY, then run the storage migration.',
    });
  }
  ...
  const { error } = await client.storage.from(env.supabase.storageBucket).upload(path, png, {
