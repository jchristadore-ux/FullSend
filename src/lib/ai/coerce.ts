...
function coerceScalar(value: unknown, node: Node, types: string[]): unknown {
  ...
  if (types.includes('string')) {
    ...
    if (isPlainObject(value)) {
      // Preferred keys first (stable, readable). Production failures showed models
      // often use other keys with *multiple* string fields -- previously only a
      // single non-preferred string was accepted, so objects with e.g. `{ what, why }`
      // were left intact and Zod rejected the array item.
      const preferred = ['name', 'label', 'title', 'capability', 'feature', 'value', 'description', 'reason', 'text', 'what', 'why', 'note'];
      ...
      const strings = Object.values(value)
        .filter((v): v is string => typeof v === 'string' && v.trim().length > 0)
        .map((v) => v.trim());
      if (strings.length >= 1) return [...new Set(strings)].join(': ');
      // Nested / non-string leaves: last-resort readable dump rather than failing.
      try {
        const dumped = JSON.stringify(value);
        if (dumped && dumped !== '{}') {
          const max = numberAt(node.maxLength);
          return max !== null && dumped.length > max ? dumped.slice(0, max) : dumped;
        }
      } catch {
        /* ignore */
      }
