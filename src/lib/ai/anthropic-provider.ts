...
export class AnthropicProvider implements AiProvider {
  ...
  private sdk(): Anthropic {
    if (!this.client) {
      ...
      // A Vercel function has a short execution window. Do not let the SDK
      // retry internally: the durable FullSend job queue owns retries, which
      // keeps one invocation from being killed before it can report failure.
  ...
  async complete(req: CompletionRequest): Promise<CompletionResponse> {
    ...
    if (req.jsonSchema) {
      system.push({
        type: 'text',
        text: 'Respond with a single JSON object and nothing else — no prose, no markdown fences. It must match this JSON Schema:\n' + JSON.stringify(req.jsonSchema),
      });
    }
    ...
    if (response.stop_reason === 'refusal') {
      throw new FullSendError('ai_refusal', 'The model declined this generation request', {
        remedy: 'Edit the campaign angle and retry.',
        meta: { task: req.task },
      });
    }
  ...
  private wrap(e: unknown, model: string): FullSendError {
    if (e instanceof Anthropic.RateLimitError) {
      return new FullSendError('ai_rate_limited', 'AI provider rate limit reached', { status: 429, retryable: true, remedy: 'FullSend will retry this generation shortly.', meta: { model }, cause: e });
    }
    if (e instanceof Anthropic.AuthenticationError) {
      return new FullSendError('ai_auth_failed', 'The Anthropic API key was rejected', { status: 401, remedy: 'Check ANTHROPIC_API_KEY in your environment.', cause: e });
    }
    if (e instanceof Anthropic.APIError) {
      const message = providerMessage(e);
      if (isBillingFailure(message)) {
        return new FullSendError('ai_billing', 'Your Anthropic account is out of credit', { status: e.status, retryable: false, remedy: 'Add credit at console.anthropic.com → Plans & Billing.', meta: { model, status: e.status }, cause: e });
      }
