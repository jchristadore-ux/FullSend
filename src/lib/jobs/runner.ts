...
const handlers: Record<JobType, Handler> = {
  analyze_repository: async (job) => {
    ...
    try {
      const result = await systemAnalyzeProduct(project, String(job.payload.repository), job.payload.githubToken ? String(job.payload.githubToken) : undefined, { refresh });
  ...
  generate_strategy: async (job) => {
    ...
    await db().update(scope, 'projects', projectId, { status: 'strategy_ready', updated_at: nowIso() }); if (project.autopilot_mode === 'full_send' && !result.strategy.approved) await db().update(scope, 'marketing_strategies', result.strategy.id, { approved: true, approved_at: nowIso() });
    await enqueueOnce(scope, 'generate_brand', { projectId }, { projectId }); return { strategyId: result.strategy.id, pillars: result.pillars.length, campaigns: result.campaigns.length, costUsd: result.costUsd, reused: result.costUsd === 0 };
  },
  generate_brand: async (job) => {
    ...
    const { brand, costUsd } = await ensureBrandProfile(scope, project, analysis, strategy, { refresh: Boolean(job.payload.refresh) }); await enqueueOnce(scope, 'generate_content', { projectId }, { projectId }); return { brandId: brand.id, costUsd, reused: costUsd === 0 };
  },
  generate_content: async (job) => {
    const scope = systemScope('job:generate_content'); const projectId = String(job.payload.projectId); const project = await db().get(scope, 'projects', projectId); if (!project) throw new Error(`Project ${projectId} not found`); const days = Number(job.payload.days ?? 30); const result = await topUpContent(scope, project, days, job.payload.brief ? String(job.payload.brief) : undefined, (job.payload.origin as never) ?? 'initial');
    if (result.generated > 0) { await db().update(scope, 'projects', projectId, { status: 'content_ready', updated_at: nowIso() }); await enqueueOnce(scope, 'schedule_content', { projectId }, { projectId }); } return { ...result };
  },
  generate_creative: async (job) => { const scope = systemScope('job:generate_creative'); const contentId = String(job.payload.contentItemId); const item = await db().get(scope, 'content_items', contentId); if (!item) throw new Error(`Content ${contentId} not found`); return { contentId, assets: item.creative_asset_ids.length }; },
  quality_control: async (job) => { const projectId = String(job.payload.projectId); const project = await db().get(systemScope('job:quality_control'), 'projects', projectId); if (!project) throw new Error(`Project ${projectId} not found`); const result = await runDailyAutopilot(projectId); return { runId: result.run.id }; },
  ...
  publish_post: async (job) => { const scope = systemScope('job:publish_post'); const outcome = await publishScheduledPost(scope, String(job.payload.scheduledPostId)); return { status: outcome.status, error: outcome.error ?? null }; },
  collect_analytics: async (job) => { const scope = systemScope('job:collect_analytics'); return { ...(await collectAnalytics(scope, String(job.payload.projectId))) }; },
  ...
  daily_autopilot: async (job) => { const result = await runDailyAutopilot(String(job.payload.projectId)); return { runId: result.run.id, published: result.published, generated: result.generated, scheduled: result.scheduled, errors: result.errors }; },
  ...
  refresh_tokens: async (job) => { const projectId = String(job.payload.projectId); const health = await checkAllConnections(projectId); return { healthy: health.healthy, needsAttention: health.needsAttention.length }; },
...
export async function drainQueue(opts: { max?: number; budgetMs?: number; projectId?: string | null } = {}): Promise<{ processed: number; succeeded: number; failed: number; dead: number }> {
  const max = opts.max ?? 20; const budgetMs = opts.budgetMs ?? 50_000; const deadline = Date.now() + budgetMs; const ROOM_TO_RUN_MS = 25_000;
  let processed = 0; let succeeded = 0; let failed = 0; let dead = 0;
...
export async function queueStats(): Promise<{ queued: number; running: number; succeeded: number; failed: number; dead: number; oldestQueuedAt: string | null }> {
  const scope = systemScope('queue stats'); const jobs = await db().find(scope, 'jobs', { orderBy: 'created_at', direction: 'asc' }); const counts = { queued: 0, running: 0, succeeded: 0, failed: 0, dead: 0 }; let oldestQueuedAt: string | null = null;
  for (const j of jobs) { if (j.status in counts) counts[j.status as keyof typeof counts]++; if (j.status === 'queued' && !oldestQueuedAt) oldestQueuedAt = j.run_after; }
  return { ...counts, oldestQueuedAt };
}

/** Accept the exact secret or the conventional Authorization: Bearer form. */
export function cronSecretValid(value: string | null): boolean {
  ...
  const prefix = 'Bearer ';
  return trimmed.startsWith(prefix) && trimmed.slice(prefix.length).trim() === secret;
}
