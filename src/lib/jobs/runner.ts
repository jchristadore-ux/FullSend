...
const handlers: Record<JobType, Handler> = {
  ...
  generate_strategy: async (job) => {
    ...
    const project = await db().get(scope, 'projects', projectId);
    if (!project) throw new Error(`Project ${projectId} not found`);
    const analysis = await db().findOne(scope, 'product_analysis', { where: { project_id: projectId }, orderBy: 'created_at', direction: 'desc' });
    ...
    const result = await buildStrategy(scope, project, analysis, personas, { refresh: Boolean(job.payload.refresh) });
  ...
  generate_brand: async (job) => {
    ...
    const strategy = await db().findOne(scope, 'marketing_strategies', { where: { project_id: projectId }, orderBy: 'version', direction: 'desc' });
    if (!strategy) throw new Error('No strategy to build a brand profile from');
    const { brand, costUsd } = await ensureBrandProfile(scope, project, analysis, strategy, { refresh: Boolean(job.payload.refresh) });
  ...
  quality_control: async (job) => { const scope = systemScope('job:quality_control'); const projectId = String(job.payload.projectId); const project = await db().get(scope, 'projects', projectId); if (!project) throw new Error(`Project ${projectId} not found`); const result = await runDailyAutopilot(projectId); return { runId: result.run.id }; },
...
export async function runJob(job: Job): Promise<{ status: 'succeeded' | 'failed' | 'dead' }> {
  ...
  try {
    const handler = handlers[job.type]; if (!handler) throw new Error(`No handler for job type ${job.type}`);
    const result = await handler(job);
    await db().update(scope, 'jobs', job.id, { status: 'succeeded', result, last_error: null, locked_at: null, updated_at: nowIso() });
    log.info('job succeeded', { id: job.id, type: job.type, ms: Date.now() - started }); return { status: 'succeeded' };
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e); const retryable = isFullSendError(e) ? e.retryable : true; const exhausted = job.attempts >= job.max_attempts || !retryable;
    if (exhausted) {
      await db().update(scope, 'jobs', job.id, { status: 'dead', last_error: message, locked_at: null, updated_at: nowIso() });
      await recordError(scope, { projectId: job.project_id, scope: `job:${job.type}`, message, remedy: isFullSendError(e) ? e.remedy : null, fatal: true });
      log.error('job dead', { id: job.id, type: job.type, attempts: job.attempts, message }); return { status: 'dead' };
    }
    const runAfter = new Date(Date.now() + backoffSeconds(job.attempts) * 1000).toISOString();
    await db().update(scope, 'jobs', job.id, { status: 'queued', last_error: message, run_after: runAfter, locked_at: null, updated_at: nowIso() });
    await recordError(scope, { projectId: job.project_id, scope: `job:${job.type}`, message, remedy: isFullSendError(e) ? e.remedy : null, fatal: false });
    log.warn('job failed, requeued', { id: job.id, type: job.type, attempts: job.attempts, runAfter, message }); return { status: 'failed' };
  }
}
...
export async function drainQueue(opts: { max?: number; budgetMs?: number; projectId?: string | null } = {}): Promise<{ processed: number; succeeded: number; failed: number; dead: number }> {
  const max = opts.max ?? 20; const budgetMs = opts.budgetMs ?? 50_000; const deadline = Date.now() + budgetMs;
  ...
  while (processed < max && Date.now() + ROOM_TO_RUN_MS < deadline) {
    const job = await db().claimNextJob(nowIso(), LOCK_TIMEOUT_MS, opts.projectId ?? null); if (!job) break;
    const { status } = await runJob(job); processed++; if (status === 'succeeded') succeeded++; else if (status === 'dead') dead++; else failed;
  }
...
/** Validates the cron secret without throwing, so cron routes can return a clean 401. */
export function cronSecretValid(value: string | null): boolean {
  const secret = env.jobs.cronSecret;
  if (!secret || !value) return false;
  const prefix = 'Bearer ';
  return value.startsWith(prefix) && value.slice(prefix.length).trim() === secret;
}
