    ...
    if (env.nodeEnv !== 'test') {
      try {
        for (const asset of assets) {
          ...
          if (ready.url !== asset.url || ready.storage_path !== asset.storage_path || ready.mime_type !== asset.mime_type) await db().update(scope, 'creative_assets', asset.id, { url: ready.url, storage_path: ready.storage_path, mime_type: ready.mime_type });
        }
