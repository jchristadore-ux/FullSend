...
export async function assertPublishable(
  ...
  //    and publishing one puts an empty image on a real feed. Only finished
  //    generation states are allowed — not merely "not failed". The scheduler
  //    already refuses these; this is the check on the last path to Instagram.
  if (!isGenerationComplete(content.generation_state)) {
    const failed = content.generation_state === 'failed';
    throw refuse(
      failed
        ? `The creative for this post was never produced: ${content.generation_error ?? 'unknown reason'}`
        : `This post is still being generated (${content.generation_state})`,
      failed
        ? 'Regenerate the creative from the post, or delete it. FullSend will not publish a post with no visual.'
        : 'Wait for generation to finish, or regenerate the creative. FullSend will not publish an unfinished post.',
    );
  }
