...
async function handlePublishError(
  ...
  if (err.retryable && attempts < maxAttempts) {
    ...
    await db().update(scope, 'scheduled_posts', post.id, {
      // Stays `scheduled` so the publish cron still claims it — but last_error
      // + attempts make the calendar chip read "Retrying", never a clean schedule.
      status: 'scheduled',
      attempts,
      last_error: err.message,
    ...
    // First failure: tell the founder now. Waiting until dead left posts looking
    // fine on the calendar for hours while nothing was going out.
    if (attempts === 1) {
      await notify(scope, {
        user_id: project.user_id,
        project_id: project.id,
        severity: 'warning',
        title: `A ${post.platform} post hit a snag — retrying`,
        body: `${err.message}${err.remedy ? ` — ${err.remedy}` : ' FullSend will retry automatically.'}`,
